...
exports.getPitchClassIntervals = getPitchClassIntervals;
exports.getPitchClassTranspositions = getPitchClassTranspositions;
exports.mergeTranspositions = mergeTranspositions;
exports.getMaqamTranspositions = getMaqamTranspositions;
exports.getJinsTranspositions = getJinsTranspositions;
const PitchClass_1 = require("../models/PitchClass");
const shiftPitchClass_1 = __importDefault(require("./shiftPitchClass"));
function getPitchClassIntervals(pitchClasses) {
    return pitchClasses.slice(1).map((pitchClass, index) => (0, PitchClass_1.calculateInterval)(pitchClasses[index], pitchClass));
}
function getPitchClassTranspositions(inputPitchClasses, jinsPitchClassIntervals, ascending, useRatio, centsTolerance) {
    const allPitchClasses = ascending ? inputPitchClasses : [...inputPitchClasses].reverse();
    const sequences = [];
    function buildSequences(pitchClasses, cellIndex, intervalIndex) {
        ...
        for (let i = cellIndex; i < allPitchClasses.length; i++) {
            ...
            const computedInterval = (0, PitchClass_1.calculateInterval)(lastCell, candidateCell);
            if (useRatio) {
...
function mergeTranspositions(ascendingSequences, descendingSequences) {
...
function getMaqamTranspositions(allPitchClasses, allAjnas, maqamTemplate, withTahlil, centsTolerance = 5, onlyOctaveOne = false) {
    ...
    if (ascendingMaqamCells.length === 0 || descendingMaqamCells.length === 0)
        return [];
    const valueType = allPitchClasses[0].originalValueType;
    const useRatio = valueType === "fraction" || valueType === "decimalRatio";
    const ascendingIntervalPattern = getPitchClassIntervals(ascendingMaqamCells);
    const descendingIntervalPattern = getPitchClassIntervals(descendingMaqamCells);
    const ascendingSequences = getPitchClassTranspositions(allPitchClasses, ascendingIntervalPattern, true, useRatio, centsTolerance).filter((sequence) => !onlyOctaveOne || sequence[0].octave === 1);
    const descendingSequences = getPitchClassTranspositions(allPitchClasses, descendingIntervalPattern, false, useRatio, centsTolerance);
    const ajnasIntervals = [];
    for (const jins of allAjnas) {
        ...
        if (jinsCells.length !== jins.getNoteNames().length)
            continue;
    ...
    const maqamTranspositions = mergeTranspositions(ascendingSequences, descendingSequences).map((sequencePair) => {
        ...
        if (sliceIndex === 0)
            sliceIndex = -1;
        const extendedAscendingPitchClasses = [...ascendingPitchClasses, ...ascendingPitchClasses.slice(sliceIndex + 1).map((pitchClass) => (0, shiftPitchClass_1.default)(allPitchClasses, pitchClass, 1))];
        const ascendingPitchClassIntervals = getPitchClassIntervals(ascendingPitchClasses);
        const ascendingMaqamAjnas = [];
        const descendingPitchClasses = [...sequencePair.descendingSequence].reverse();
        const extendedDescendingPitchClasses = [...descendingPitchClasses, ...descendingPitchClasses.slice(sliceIndex + 1).map((pitchClass) => (0, shiftPitchClass_1.default)(allPitchClasses, pitchClass, 1))];
        const descendingPitchClassIntervals = getPitchClassIntervals(descendingPitchClasses);
        const descendingMaqamAjnas = [];
        ...
        if (allAjnas.length > 0) {
            for (let i = 0; i < extendedAscendingPitchClassIntervals.length; i++) {
                ...
                for (const jinsInterval of ajnasIntervals) {
                    ...
                    if ((0, PitchClass_1.matchingListOfIntervals)(extendedAscendingPitchClassIntervals.slice(i, i + lengthOfInterval), jinsInterval.intervals, centsTolerance)) {
                ...
                if (!found)
                    ascendingMaqamAjnas.push(null);
                if (ascendingMaqamAjnas.length - 1 === ascendingPitchClassIntervals.length)
                    break;
            }
            for (let i = 0; i < extendedDescendingPitchClassIntervals.length; i++) {
                ...
                for (const jinsInterval of ajnasIntervals) {
                    const lengthOfInterval = jinsInterval.intervals.length;
                    if ((0, PitchClass_1.matchingListOfIntervals)(extendedDescendingPitchClassIntervals.slice(i, i + lengthOfInterval), jinsInterval.intervals, centsTolerance)) {
                        const jins = jinsInterval.jins;
                        const firstJinsNote = jins.getNoteNames()[0];
                ...
                if (!found)
                    descendingMaqamAjnas.push(null);
                if (descendingMaqamAjnas.length - 1 === descendingPitchClassIntervals.length)
                    break;
            }
        }
        return {
...
function getJinsTranspositions(allPitchClasses, jinsTemplate, withTahlil, centsTolerance = 5, onlyOctaveOne = false) {
    ...
    const useRatio = valueType === "fraction" || valueType === "decimalRatio";
    const intervalPattern = getPitchClassIntervals(jinsCells);
    const jinsTranspositions = getPitchClassTranspositions(allPitchClasses, intervalPattern, true, useRatio, centsTolerance)
    ...
    if (withTahlil && tahlilTransposition)
        return [{ ...tahlilTransposition, transposition: false }, ...jinsTranspositionsWithoutTahlil];
    else
        return jinsTranspositionsWithoutTahlil;
}
