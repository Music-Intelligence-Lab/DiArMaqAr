...
function buildScalaHeader(
  ...
  const baseUrl = metadata.baseUrl || 'https://diarmaqar.net';
...
export function exportMaqamTo12ToneScala(
  ...
  // Extract base URL from currentUrl (e.g., "http://localhost:3000" or "https://diarmaqar.net")
  let baseUrl = 'https://diarmaqar.net'; // Default fallback
  if (currentUrl) {
    try {
...
function buildTuningSystemKeymapContent(
  ...
  const lines = [
    `! ${desc}`,
    `! Generated by the Digital Arabic Maqām Archive (DiArMaqAr) https://diarmaqar.net`,
    `!`,
