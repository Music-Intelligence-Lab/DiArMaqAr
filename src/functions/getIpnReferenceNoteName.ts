import { calculate12EdoReferenceMidiNote } from "./calculateIpnReferenceMidiNote";
 ...
 *
 * @param pitchClass - Pitch class object with midiNoteNumber and optional referenceNoteName
 * @returns 12-EDO reference note name without octave number (e.g., "E", "Bb", "F#")
 *
 * @example
 * ```typescript
 * const pc = { midiNoteNumber: 51.366, englishName: "E-b3", ... };
 * const ref = get12EdoReferenceNoteName(pc); // Returns "E"
 * ```
 */
export function getIpnReferenceNoteName(pitchClass: any): string {
  const midiNote = calculate12EdoReferenceMidiNote(pitchClass);
 ...
 *
 * @param pitchClass - Pitch class object with midiNoteNumber
