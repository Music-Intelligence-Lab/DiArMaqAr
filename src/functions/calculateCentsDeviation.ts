...
function parseEnglishNoteName(englishName: string): { baseNote: string; chromaticSemitones: number; mainAccidental: string } {
  ...
  // Look for the main accidental (b or #)
  // Handle cases like "b", "#", "-b" (microtonal flat), "-#" (unlikely but possible), "+b", "+#"
  // The pattern "-b" or "+b" means a microtonally-altered flat, so the main accidental is still "b"
  // The pattern "-#" or "+#" means a microtonally-altered sharp, so the main accidental is still "#"
