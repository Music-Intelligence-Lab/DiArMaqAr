...
export async function GET() {
  try {
    ...
    const errorResponse = new NextResponse("Failed to load Sources.", { status: 500 });
...
export async function PUT(request: Request) {
  try {
    ...
    const response = NextResponse.json({ message: "Sources updated successfully." });
    return addCorsHeaders(response);
  } catch (error) {
    console.error("Error updating Sources:", error);
    const errorResponse = new NextResponse("Failed to update Sources.", { status: 500 });
    return addCorsHeaders(errorResponse);
  }
}
