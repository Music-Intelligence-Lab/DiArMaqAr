...
const WheelCell = React.memo<WheelCellProps>(
  ({ pitchClass, isSelected, isActive, isDescending, isTonic, isCurrentTonic, mapping, inputType, isBlackKey, isMapped, onClick }) => {
    ...
    if (inputType === "MIDI" && isTonic) {
      if (isBlackKey) {
      className = baseClassName + " pitch-class-wheel__cell_midi_tonic_black";
      } else {
      className = baseClassName + " pitch-class-wheel__cell_midi_tonic_white";
      }
    }
    
    if (inputType === "MIDI" && isCurrentTonic) {
      className = baseClassName + " pitch-class-wheel__cell_midi_tonic_current";
    }

    if (inputType === "MIDI" && isCurrentTonic && isActive) {
      className = baseClassName + " pitch-class-wheel__cell_midi_tonic_current_active";
    }
