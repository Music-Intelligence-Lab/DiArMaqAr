...
export default function JinsTranspositions() {
  ...
  const transpositionTables = useMemo(() => {
    ...
    function renderTransposition(jins: Jins, index: number) {
      ...
      return (
        <>
          <tr className="jins-transpositions__header" id={getJinsHeaderId(pitchClasses[0]?.noteName)} style={index === 0 || index === 1 ? { scrollMarginTop: "140px" } : undefined}>
  ...
  useEffect(() => {
    ...
    if (jinsFirstNote) {
      setTimeout(() => {
        scrollToJinsHeader(decodeURIComponent(jinsFirstNote), selectedJinsDetails);
      }, 200);
    }
  }, [selectedJinsDetails]);
